...
const CreateQuote: React.FC = () => {
  ...
  const handleItemChange = (idx: number, patch: Partial<QItem & { lineDiscountMode?: 'PERCENT' | 'AMOUNT' }>) => {
    setItems(prev =>
      prev.map((item, i) => {
        ...
        updatedItem.itemCost = Math.max(0, updatedItem.itemCost || 0);
        
        if (patch.hasOwnProperty('itemCost')) {
          updatedItem.itemRate = Math.max(updatedItem.itemCost, updatedItem.itemRate || 0);
        } else {
          updatedItem.itemRate = Math.max(0, updatedItem.itemRate || 0);
        }

        if (updatedItem.lineDiscountMode === 'PERCENT') {
          updatedItem.lineDiscountPercent = Math.max(0, Math.min(updatedItem.lineDiscountPercent || 0, 100));
        } else if (updatedItem.lineDiscountMode === 'AMOUNT') {
          const gross = (updatedItem.quantity || 0) * (updatedItem.itemRate || 0);
          updatedItem.lineDiscountAmount = Math.max(0, Math.min(updatedItem.lineDiscountAmount || 0, gross));
        }
  ...
  const handleOverallDiscountChange = (value: number) => {
    ...
    if (discountMode === 'PERCENT') {
      ...
      finalValue = Math.min(finalValue, totals.subtotal);
    }
              ...
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                <div>
                  ...
                  <input
                    ...
                    readOnly
                    value={leadNumber}
                    className="w-full h-11 px-4 rounded-xl border border-cloud-300/50 dark:border-midnight-600/50 bg-white/70 dark:bg-midnight-800/60 text-midnight-900 dark:text-ivory-100 shadow-sm focus:border-sky-400 focus:ring-2 focus:ring-sky-300/50 cursor-pointer transition"
              ...
              <div className="border-t border-cloud-300/40 dark:border-midnight-700/40 pt-6">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  ...
                  <div className="sm:col-span-3">
                    ...
                    <textarea
                      rows={3}
                      className="w-full px-4 py-3 rounded-xl border border-cloud-300/50 dark:border-midnight-600/50 bg-white/70 dark:bg-midnight-800/60 text-midnight-900 dark:text-ivory-100 shadow-sm focus:border-sky-400 focus:ring-2 focus:ring-sky-300/50 transition"
                      value={description}
                ...
                >
                  ...
                  <tbody className="divide-y divide-cloud-300/30 dark:divide-midnight-700/30">
                    {items.map((item, idx) => {
                      ...
                      const isRateInvalid = item.itemRate < item.itemCost;
                        ...
                        >
                          ...
                          <td className="px-4 py-2 text-right">
                            <input
                              ...
                              title={isRateInvalid ? "Rate cannot be lower than Cost" : ""}
                            />
                          </td>
                          <td className="px-4 py-2">
                            <select
                          ...
                          <td className="px-4 py-2 text-right">
                            <input
                              ...
                              onChange={(e) =>
                                handleItemChange(idx, {
                                  [item.lineDiscountMode === 'PERCENT' ? 'lineDiscountPercent' : 'lineDiscountAmount']: Number(e.target.value)
                                })
                              }
                            />
                          </td>
