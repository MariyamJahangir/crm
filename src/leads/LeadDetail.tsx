...
const QuotePicker: React.FC<{
  ...
  const downloadFromPreview = async () => {
    ...
    try {
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = `${preview.quote.quoteNumber}.pdf`;
      document.body.appendChild(a);
      a.click();
      a.remove();
      setTimeout(() => window.URL.revokeObjectURL(url), 1200);
  ...
  const selectMain = async (q: Quote) => {
    ...
    onMainChange(q.quoteNumber); // optimistic update
    setBusy(q.id);
  ...
  if (!quotes.length)
    return <div className="text-sm text-midnight-400 dark:text-ivory-500 italic">No quotes yet.</div>;
            ...
            <>
              ...
              <div className="bg-cloud-50/30 dark:bg-midnight-900/30 backdrop-blur-xl border border-cloud-300/30 dark:border-midnight-700/30 rounded-2xl p-5 shadow-lg mb-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm text-midnight-700 dark:text-ivory-200">
                  ...
                  <div><span className="font-medium text-midnight-500 dark:text-ivory-400">Creator:</span> {lead.creatorType ? `${lead.creatorType} (${lead.creatorId})` : '-'}</div>
                </div>
                {lead.previewUrl && (
                  <img
                    src={lead.previewUrl}
